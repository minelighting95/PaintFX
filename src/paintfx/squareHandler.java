...
public class squareHandler implements EventHandler<ActionEvent> {
    ...
    /**
     * Function to set phase of squareHandler (Normally 2)
     ...
     */
    public squareHandler(StackPane coolCrab, ColorPicker colorPicker, ColorPicker fillPick, TextField widthText, ToggleButton rectBtn, Stage primaryStage, TabPane tabPane, CheckBox fillBox, ToggleButton dropBtn, ToggleButton dropBtn1){
        ...
        canTemp.setOnMouseDragged((event) ->{
            if(hold == 0){                                                 // 2nd time
                ...
                if((x2 < x1) && (y2 < y1)){
                    ...
                    if(fillBox.isSelected()){
                        gcTemp.fillRect(x1, y2, width, height);                     // Create square between user selected points
                    }  
                }
                else if(x2 < x1){
                    width = x1-x2;
