...
public class ovalHandler implements EventHandler<ActionEvent> {
    ...
    public ovalHandler(Canvas canvas, Canvas canvas1, Canvas canvas2, GraphicsContext gc, GraphicsContext gc1, GraphicsContext gc2,
        ...
        if(PaintFX.getSelec() == 0){
            ...
            if(hold == 0){                                                      // 1st time
                ...
                if(dropBtn.isSelected()){                                       // Get Color
                    ...
                    gc.setStroke(colorPicker.getValue());                       // Set Ellipse Color to selected value
                }
                if(dropBtn1.isSelected()){                                      // Get Fill Color
                    gc.setFill(PaintFX.getColor1());
                }
                else{
                    gc.setFill(fillPick.getValue());                            // Set Ellipse Color to selected value
                }
                ...
                if((x2 < x1) && (y2 < y1)){
                    gc.strokeOval(x2, y2, x1-x2, y1-y2);                        // Create ellipse between user selected points
                    if(fillBox.isSelected()){
                        gc.fillOval(x2, y2, x1-x2, y1-y2);                      // Create ellipse between user selected points
                    }
                }
                else if(y2 < y1){
                    gc.strokeOval(x1, y2, x2-x1, y1-y2);                        // Create ellipse between user selected points
                    if(fillBox.isSelected()){
                        gc.fillOval(x1, y2, x2-x1, y1-y2);                      // Create ellipse between user selected points
                    }  
                }
                else if(x2 < x1){
                    gc.strokeOval(x2, y1, x1-x2, y2-y1);                        // Create ellipse between user selected points
                    if(fillBox.isSelected()){
                        gc.fillOval(x2, y1, x1-x2, y2-y1);
                    }
                }
                else{
                    gc.strokeOval(x1, y1, x2-x1, y2-y1);                        // Create ellipse between user selected points
                    if(fillBox.isSelected()){
            ...
            if(hold == 0){                                                      // 1st time
                ...
                hold = 2;
                if(dropBtn.isSelected()){                                       // Get Color
                    gc1.setStroke(PaintFX.getColor1());
                }
                else{
                    gc1.setStroke(colorPicker.getValue());                      // Set Ellipse Color to selected value
                }
                if(dropBtn1.isSelected()){                                      // Get Fill Color
                    gc1.setFill(PaintFX.getColor());    
                }
                else{
                ...
                gc1.setLineWidth(lineWidth);                                    // Set Ellipse to desired width
                
                if((x2 < x1) && (y2 < y1)){
                    gc1.strokeOval(x2, y2, x1-x2, y1-y2);                       // Create Ellipse between user selected points
                    if(fillBox.isSelected()){
                        gc1.fillOval(x2, y2, x1-x2, y1-y2);                     // Create Ellipse between user selected points
                    }
                }
                else if(y2 < y1){
                    gc1.strokeOval(x1, y2, x2-x1, y1-y2);                       // Create Ellipse between user selected points
                    if(fillBox.isSelected()){
                        gc1.fillOval(x1, y2, x2-x1, y1-y2);                     // Create Ellipse between user selected points
                    }  
                }
                else if(x2 < x1){
                    gc1.strokeOval(x2, y1, x1-x2, y2-y1);                       // Create Ellipse between user selected points
                    if(fillBox.isSelected()){
                        gc1.fillOval(x2, y1, x1-x2, y2-y1);
                    }
                }
                else{
                    gc1.strokeOval(x1, y1, x2-x1, y2-y1);                       // Create Ellipse between user selected points
                    if(fillBox.isSelected()){
                        gc1.fillOval(x1, y1, x2-x1, y2-y1);
                    }
                }
            ...
            if(hold == 0){                                                      // 1st time
                ...
                gc2.setLineWidth(lineWidth);                                    // Set ellipse to desired width
                
                if((x2 < x1) && (y2 < y1)){
                    gc2.strokeOval(x2, y2, x1-x2, y1-y2);                       // Create Ellipse between user selected points
                    if(fillBox.isSelected()){
                        gc2.fillOval(x2, y2, x1-x2, y1-y2);                     // Create Ellipse between user selected points
                    }
                }
                else if(y2 < y1){
                    gc2.strokeOval(x1, y2, x2-x1, y1-y2);                       // Create Ellipse between user selected points
                    if(fillBox.isSelected()){
                        gc2.fillOval(x1, y2, x2-x1, y1-y2);                     // Create Ellipse between user selected points
                    }  
                }
                else if(x2 < x1){
                    gc2.strokeOval(x2, y1, x1-x2, y2-y1);                       // Create Ellipse between user selected points
                    if(fillBox.isSelected()){
                        gc2.fillOval(x2, y1, x1-x2, y2-y1);
                    }
                }
                else{
                    gc2.strokeOval(x1, y1, x2-x1, y2-y1);                       // Create Ellipse between user selected points
                    if(fillBox.isSelected()){
