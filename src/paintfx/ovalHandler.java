...
/**
 * Event Handler to Draw an Oval Onscreen
 * 
 * @author Alex Appel
 */
public class ovalHandler implements EventHandler<ActionEvent> {
    ...
    /**
     * Set phase of ovalHandler (Normally 2)
    ...
    /**
     * Event Handler to Place an Oval on Screen
     ...
     * @param rectBtn Oval Handler Button
     ...
     */
    public ovalHandler(StackPane coolCrab, ColorPicker colorPicker, ColorPicker fillPick, TextField widthText, ToggleButton rectBtn, TabPane tabPane, CheckBox fillBox, ToggleButton dropBtn, ToggleButton dropBtn1){
    ...
    public void handle(ActionEvent a) {
        ...
        hold = 0;                                                               // Set Holds
        PaintFX.logItem("Ellipse", 1);                                          // Log Save
        ...
        if(PaintFX.getSelec() == 0){                                            // For selected tab,
            image = PaintFX.canvasPeek().snapshot(params, null);                // Take screenshot and draw image
            ...
            if(hold == 0 && rectBtn.isSelected()){                              // If mouse dragged
                ...
                if((x2 < x1) && (y2 < y1)){
                    ...
                    if(fillBox.isSelected()){
                        gcTemp.fillOval(x2, y2, x1-x2, y1-y2);                  // Fill ellipse between user selected points
                    }
                }
                else if(y2 < y1){
                    gcTemp.strokeOval(x1, y2, x2-x1, y1-y2);                    // Create ellipse between user selected points
                    if(fillBox.isSelected()){
                        gcTemp.fillOval(x1, y2, x2-x1, y1-y2);                  // Fill ellipse between user selected points
                    }  
                }
                else if(x2 < x1){
                    gcTemp.strokeOval(x2, y1, x1-x2, y2-y1);                    // Create ellipse between user selected points
                    if(fillBox.isSelected()){
                        gcTemp.fillOval(x2, y1, x1-x2, y2-y1);                  // Fill ellipse between user selected points
                    }
                }
                else{
                    gcTemp.strokeOval(x1, y1, x2-x1, y2-y1);                    // Create ellipse between user selected points
                    if(fillBox.isSelected()){
                        gcTemp.fillOval(x1, y1, x2-x1, y2-y1);                  // Fill ellipse between user selected points
                    }
                }
            }
        });
