...
public class dropBtnHandler implements EventHandler<ActionEvent> {
    ...
    public dropBtnHandler(ImageView cool, ImageView cool1, ImageView cool2, StackPane coolCrab, Canvas canvas, 
        ...
        if(PaintFX.getSelec() == 0){                                                     
            canvas.setOnMouseClicked((event) ->{                            // When mouse clicked
                ...
                if((wim.getPixelReader().getColor((int)x, (int)y)) == Color.TRANSPARENT){
                    ...
                    PaintFX.setColor(r.getColor((int)x, (int)y));
                }
                else{
                    PaintFX.setColor(wim.getPixelReader().getColor((int)x, (int)y));   // If not, grab canvas color
                }
            });
        }
